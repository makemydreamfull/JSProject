...
export class CreationCategoriesIncome {
    ...
    async clickBtn() {
        ...
        if (result.error || result.redirect || !result.response || (result.response && (result.response.error || result.response.message))) {
            ...
            return alert('Произошла ошибка в создании категории доходов. Если вам необходимо создать данную категорию, обратитесь в поддержку!')
        }
