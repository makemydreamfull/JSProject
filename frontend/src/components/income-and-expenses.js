...
export class IncomeAndExpenses {
    ...
    constructor(openNewRoute) {
        ...
        // this.arr = []
        // if (CreationIncomeAndExpenses.typeFormLine) {
        //
        //     this.infoTable = CreationIncomeAndExpenses.typeFormLine
        //     this.arr.push(this.infoTable)
        // }
        // Как сделать при помощи массива ?
        document.getElementById('income-link').addEventListener('click', this.clickLinkIncome.bind())
        document.getElementById('expenses-link').addEventListener('click', this.clickLinkIncome.bind())
        this.getOperations()
    }
    ...
    async getOperations() {
        ...
        for (let i = 0; i < result.response.length; i++) {
            ...
            svgTdElements.innerHTML = `<svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg" class="trash me-2" id="trash-` + result.response[i].id + `">
               ...
               <a href="/editing-income-and-expenses?id=`+ result.response[i].id +`"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" class="edit" id="edit-` + result.response[i].id + `">
    ...
    async deleteElements(event) {
        ...
        if(dataLocalStorage){
            const data  = JSON.parse(dataLocalStorage).splice(idOperation + 1,1)
